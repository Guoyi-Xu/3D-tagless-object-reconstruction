...
public class MyDesign29 implements LLRPEndpoint {

	private LLRPConnection connection;

	private static Logger logger  = Logger.getLogger(MyDesign29.class.getName());
		...
		try {
			...
			if (status.equals(new StatusCode("M_Success"))) {
				...
				if ((dev_cap == null) || (!dev_cap.getDeviceManufacturerName().equals(new UnsignedInteger(25882)))) {
					logger.error("MyDesign29 must use Impinj model Reader, not " + dev_cap.getDeviceManufacturerName().toString());
					System.exit(1);
				}
	...
	private void logOneTagReport(TagReportData tr) {
		...
		int msg_freq_int = msg_freq.intValue();
		if (msg_freq_int == 905250) {
			chindexlist.add(new Double(1));
		} else if (msg_freq_int == 909250) {
			chindexlist.add(new Double(2));
		} else if (msg_freq_int == 913250) {
			chindexlist.add(new Double(3));
		} else if (msg_freq_int == 917250) {
			chindexlist.add(new Double(4));
		} else if (msg_freq_int == 921250) {
			chindexlist.add(new Double(5));
		} else if (msg_freq_int == 925250) {
			chindexlist.add(new Double(6));
		}
		switch (msg_freq_int) {
			case 905250:
			case 909250:
			...
			case 925250:
				...
				if (epc_str.contains("300833B2DDD9014000004033")) {
				...
				} else if (epc_str.contains("300833B2DDD9014000004001")) {
    ...
    public static void main(String[] args) throws IOException, WriteException
    {
    	...
    	MyDesign29 masterpiece = new MyDesign29();
    	...
    	while (cnt <= 1) {
    		...
    		for (int i = s1; i < s2; i ++) {
    			clist_d[i - s1] = chindexlist.get(i);
    		...
    		MLDouble mlDouble1 = new MLDouble("chindexlist", clist_d, count);
    		...
    		list.add(mlDouble5);
    		list.add(mlDouble6);
    		String data_name = new String("C:\\Users\\Vicon-OEM\\Desktop\\Kan_Group_folder\\reader_apps\\project29\\data_wo_Pragya_standing_x6y6_rm0.mat");
    	...
    	while (cnt <= 2) {
    		...
    		list.add(mlDouble5);
    		list.add(mlDouble6);
    		String data_name = new String("C:\\Users\\Vicon-OEM\\Desktop\\Kan_Group_folder\\reader_apps\\project29\\data_w_Pragya_standing_x6y6_v" + String.valueOf(cnt) + "_rm0.mat");
    		new MatFileWriter(data_name, list);
    		System.out.println("Write to .mat file success.");
    	...
    	while (rowid < epclist.size() + 1) {
    		...
    		cell = row.createCell(0);
    		cell.setCellValue(chindexlist.get(rowid - 1).toString());
    		cell = row.createCell(1);
    		...
    		cell.setCellValue(chindlist.get(rowid - 1));
    		rowid += 1;
    	}
    	...
    	FileOutputStream out = new FileOutputStream(new File("C:\\Users\\Vicon-OEM\\Desktop\\Kan_Group_folder\\reader_apps\\project29\\result_data.xlsx"));
    	workbook.write(out);
    	System.out.println("result_data.xlsx written successfully"); 


    	// Write to .mat file. (to be compared with the excel file written to)
   	 	/*int count = chindexlist.size();
    	double[] clist_d = new double[count];
    	double[] tlist_d = new double[count];
    	double[] alist_d = new double[count];
    	...
    	for (int i = 0; i < count; i ++) {
    		clist_d[i] = chindexlist.get(i);
    		tlist_d[i] = tagindexlist.get(i);
    		alist_d[i] = antennalist.get(i);
    		rlist_d[i] = rssiimpinjlist.get(i);
    		rdlist_d[i] = rssiimpinjlist_d.get(i);
    		plist_d[i] = phasedeglist.get(i);
    	}
    	...
    	list.add(mlDouble5);
    	list.add(mlDouble6);
    	new MatFileWriter("data_w_Pragya_standing_x6y6_v1-2_rm0.mat", list);
    	System.out.println("Write to .mat file success."); */
